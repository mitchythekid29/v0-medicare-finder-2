...
export default function MedicareAdvantageLanding() {
  ...
  // Load TrustedForm and check when ActiveProspect.track is available
  useEffect(() => {
    ...
    const waitForAP = setInterval(() => {
      if (window.ActiveProspect?.track) {
        console.log("[✔️] ActiveProspect.track is available")
        setApReady(true)
        clearInterval(waitForAP)
      } else {
        console.log("[...] Waiting for ActiveProspect.track to be available...")
      }
    }, 500)
